  ...
  function checkAchievements() {
    ...
    for (var j = 0; j < ACHIEVEMENT_DEFS.length; j++) {
      ...
      if (checker[a.id] && checker[a.id](d)) { 
        ...
        newUnlocks.push(a); 
        if (a.bonus) bonusUnlocks.push(a);
        playAchSound = true;
      }
    }
    if (newUnlocks.length > 0) storeSet('achievements_v1', achievements);
    for (var b = 0; b < bonusUnlocks.length; b++) {
      earnCoins(bonusUnlocks[b].bonus, 'Награда: ' + bonusUnlocks[b].name, true);
    }
    if (playAchSound) Juice.playAchievement();
    if (newUnlocks.length > 0) { window.dispatchEvent(new CustomEvent('achievement-unlocked', { detail: newUnlocks })); }
    return achievements;
  }
