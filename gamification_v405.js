(function () {
  ...
  function addReward(name, price, icon) { 
    ...
    var existingIds = {};
    for (var i = 0; i < storeData.customRewards.length; i++) {
      var existing = storeData.customRewards[i];
      if (existing && typeof existing.id === 'string') existingIds[existing.id] = true;
    }
    var id = makeLegacyRewardId('custom', existingIds);
    var rewardName = typeof name === 'string' ? name : 'Награда';
    var rewardPrice = parsePositiveInt(price, 10, 1);
    var data = storeData;
    data.customRewards.push({ id: id, name: rewardName, price: rewardPrice, icon: icon || '🎁', expiresIn: 24 });
    storeSet('store_v2', data);
    return data;
  }
